...
class BtcStreamManager:
    ...
    async def start(self):
        ...
        # Kalshi WS — redundant connection pool
        if kalshi_ws_available():
            log.info("Kalshi WebSocket auth configured — using live streaming")
            self._ks_pool = RedundantWSPool(WSPoolConfig(
                name="ks_market",
                url=KALSHI_WS_URL,
                pool_size=2,
                auth_headers=_kalshi_ws_auth_headers,
                subscribe_msgs=self._ks_subscribe_msgs,
                ping_text=None,  # server-initiated pings, library auto-responds
                recv_timeout=60.0,
                reconnect_base=0.5,
                reconnect_max=5.0,
                on_message=self._handle_ks_ws_msg,
            ))
    ...
    def _sync_orch_prices(self):
        ...
        # PM taker fee rate: 0.072 for crypto (from docs.polymarket.com/trading/fees)
        fee_schedule = pm.get("fee_schedule")
        pm_fee_rate = 0.072
        if isinstance(fee_schedule, dict):
            pm_fee_rate = float(fee_schedule.get("rate", 0.072))
        self._model_orch.set_prices(
            ks_yes_ask, ks_no_ask, pm_up_ask, pm_down_ask, pm_fee_rate,
        )
    ...
    def _ks_subscribe_msgs(self) -> list[str]:
        """Build Kalshi subscribe messages for the pool. Called per connection."""
        msgs = []
    ...
    async def _handle_ks_ws_msg(self, raw: str):
        """Handle a deduplicated Kalshi WS message from the pool."""
        try:
            msg = json.loads(raw)
    ...
    async def _rtds_ws_loop(self):
        ...
        while self._running:
            try:
                log.info("Connecting to PM RTDS for Chainlink BTC/USD")
    ...
    async def _rtds_recv_loop(self, ws):
        """Receive Chainlink BTC/USD price updates from RTDS."""
        import websockets
        while self._running:
            try:
