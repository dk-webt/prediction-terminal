...
function HelpView() {
  const CMDS = [
    ...
    ['BTC GRAPHS DBG', 'Toggle on-screen model debug overlay (ADF/OU, copula, EV, gates)'],
    ['BTC GRAPHS DBG ON/OFF', 'Force model debug overlay on or off'],
    ['BTC LOG PM ON/OFF', 'Start/stop logging PM bid/ask CSV (pm_price_data.log)'],
    ['BTC LOG PM', 'Download the PM price CSV (also accepts BTC LOG PM_PRICE_DATA)'],
    ['BTC LOG PM CLEAR', 'Clear the PM price log file'],
    ['BTC REFRESH', 'Re-fetch contracts + swap subscriptions in-place'],
    ['BTC REFRESH FORCE', 'Re-fetch contracts + full WebSocket reconnect (nuclear option)'],
