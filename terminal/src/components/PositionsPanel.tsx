...
export default function PositionsPanel({ style }: Props) {
  ...
  const hasKs = positions.kalshi.length > 0
  const hasPm = positions.polymarket.length > 0
  const hasAny = hasKs || hasPm
  ...
  return (
    <div className="events-panel-dynamic" style={style}>
      ...
      <div className="panel-body" style={{ overflow: 'auto' }}>
        ...
        {!positions.loading && !hasAny && !positions.error && (
          <div style={{ color: 'var(--amber-dim)', padding: 8, fontSize: 11 }}>
            No open positions
          </div>
        )}
        ...
        {hasPm && (
          <>
            <div style={{ color: 'var(--amber)', padding: '6px 8px 2px', fontSize: 10, letterSpacing: 1, marginTop: hasKs ? 8 : 0 }}>
              POLYMARKET
            </div>
            <table className="pos-table">
              <thead>
